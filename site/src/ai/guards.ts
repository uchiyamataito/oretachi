...
export const CRISIS_TEMPLATE =
  'つらい気持ちを話してくださってありがとうございます。いま、あなたの安全がいちばん大切です。' +
...
// 封印の返しは「まず参考記事→やわらかく個別事情→専門家/窓口は最後」の順（doc43§14＝“できません”を先頭に置かない）。
const SEALED_TEMPLATES: Record<SealedTopic, string> = {
  money_zaisan:
    '財産分与の考え方は「財産分与の対象・対象外」の記事にまとめていますので、まずはこちらが参考になるかもしれません。' +
    'ただ、具体的な金額は財産の内容や個別の事情で変わるため、このチャットでは断定せず、正確なところは弁護士・公的窓口での確認をおすすめします。',
  money_isharyo:
    '慰謝料の一般的な考え方は関連記事にまとめていますので、まずはそちらが参考になるかもしれません。' +
    'ただ、金額はケースごとの事情で変わるため、このチャットでは断定せず、具体的な額は専門家・公的窓口にご相談ください。',
  money_yoikuhi:
    '養育費は、相場の考え方と計算ツールを「養育費」の記事にまとめていますので、まずはそちらが参考になるかもしれません。' +
    'ただ、正確な金額は年収やお子さんの人数などで変わるため、このチャットでは断定せず、算定表や専門家でのご確認をおすすめします。',
  nenkin_bunkatsu:
    '年金分割の手続きの流れは関連記事にまとめていますので、まずはそちらが参考になるかもしれません。' +
    'ただ、対象や割合の法的な判断はこのチャットではできないため、個別の可否は日本年金機構・専門家にご確認ください。',
  shinken_outlook:
    '「父親の親権」の記事に、条件と今からできることをまとめていますので、まずはそちらが参考になるかもしれません。' +
    'ただ、親権が取れるかどうかの見通しは個別の事情で変わるため、このチャットでは断定せず、個別のご相談は専門家へお願いします。',
};
...
const PII_REFUSE_TEMPLATE =
  '安全のため、口座番号・カード番号・マイナンバーなどの個人情報は入力しないでください。' +
  'それらがなくてもご相談いただけます。番号を消して、もう一度送っていただけると助かります。';
